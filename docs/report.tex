...
% ── TOC spacing (reduce vertical gaps by 2pt) ───────────
\setlength{\cftbeforechapskip}{4pt}   % default ~10pt, reduced
\setlength{\cftbeforesecskip}{1pt}    % default ~3pt, reduced
\setlength{\cftbeforesubsecskip}{1pt} % default ~3pt, reduced
...
\noindent I hereby declare that the thesis entitled ``Semantic Cache for LLM Serving'' submitted by \textbf{Pojesh Kumar R (22BAI1373)}, for the award of the degree of \mbox{Bachelor of Technology} in \mbox{Computer Science} and Engineering, Vellore Institute of Technology, Chennai is a record of bonafide work carried out by me under the supervision of \textbf{\mbox{Dr.~Kumar~R}}.
...
\noindent This is to certify that the report entitled ``\textbf{Semantic Cache for LLM Serving}'' is prepared and submitted by \textbf{Pojesh Kumar R (22BAI1373)} to Vellore Institute of Technology, Chennai, in partial fulfillment of the requirement for the award of the degree of \textbf{\mbox{Bachelor of Technology} in \mbox{Computer Science} and Engineering (\mbox{Artificial Intelligence} and \mbox{Machine Learning})} is a bonafide record carried out under my guidance. The project fulfills the requirements as per the regulations of this University and in my opinion meets the necessary standards for submission. The contents of this report have not been submitted and will not be submitted either in part or in full, for the award of any other degree or diploma and the same is certified.

\vspace{2cm}

\noindent Signature of the Guide:\\
Name: \mbox{Dr.~Kumar~R}\\
Date: 
...
Name: \mbox{Dr.~Tulasi~Prasad~Sariki}\\
Date: \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \
\end{flushright}
...
\noindent It is my pleasure to express with deep sense of gratitude to \textbf{\mbox{Dr.~Kumar~R}}, School of \mbox{Computer Science} and Engineering, Vellore Institute of Technology, Chennai, for his constant guidance, continual encouragement, understanding; more than all, he taught me patience in my endeavor. My association with him is not confined to academics only, but it is a great opportunity on my part to work with an intellectual and expert in the field of machine learning and distributed systems.
...
\noindent Special mention to Dr.~Viswanathan~V, Dean, Dr.~Nithyanandam~P, Dr.~Suganya~G, Dr.~Sweetlin~Hemalatha~C, Associate Deans, School of \mbox{Computer Science} and Engineering, Vellore Institute of Technology, Chennai for spending their valuable time and efforts in sharing their knowledge and for helping us in every aspect.

\vspace{0.3cm}

\noindent In jubilant state, I express ingeniously my whole-hearted thanks to \textbf{\mbox{Dr.~Tulasi~Prasad~Sariki}}, Head of the Department, B.Tech.\ CSE AIML and the Project Coordinators for their valuable support and encouragement to take up and complete the thesis.
...
SCALM & Semantic Caching for Automated Chat Services with LLMs \\
...
\begin{longtable}{|c|p{2.5cm}|p{2.5cm}|p{4cm}|p{4cm}|}
\hline
...
\hline
\endhead
1 & Haqiq et al., 2025 & MinCache: Hybrid Cache & Introduces a three-tier matching hierarchy combining exact, resemblance, and semantic matching. Achieves 4.5$\times$ speedup over GPTCache by filtering candidates at cheaper tiers before invoking full semantic comparison. & Relies on MinHash for resemblance tier, which captures surface-level token overlap but misses nuanced semantic relationships between paraphrased queries. No per-entry threshold adaptation. \\
\hline
2 & Gill et al., 2024 & MeanCache: User-Centric Caching & Proposes user-centric semantic caching with federated learning for training similarity models across multiple clients without sharing raw data. Uses PCA-based embedding compression to reduce storage and an adaptive threshold computed as mean minus one standard deviation. & The adaptive threshold operates at a global level and cannot distinguish between query types requiring different precision levels. No admission control or cost-aware eviction. \\
\hline
3 & Gao et al., 2025 & Online Context Caching & Addresses KV cache placement and request scheduling for distributed LLM serving. Optimizes which attention key-value tensors to cache across GPU memory hierarchies to minimize inference latency. & Focuses on internal model computation caching rather than semantic similarity between user queries. Does not address query-level caching or threshold learning. \\
\hline
4 & Li et al., 2024 & SCALM: Semantic Caching & Applies vector quantization and semantic clustering to group similar queries before cache lookup. Achieves significant token savings by serving clustered responses for queries within the same semantic group. & The clustering and vector quantization introduce non-trivial computational overhead. No mechanism for adapting similarity thresholds based on feedback, and cluster boundaries remain static. \\
\hline
5 & Li et al., 2024 & Context-based Semantic Caching & Enriches query embeddings with contextual information using attention-weighted fusion of surrounding conversation turns. Improves cache hit accuracy for multi-turn dialogues where individual queries lack sufficient context. & The attention-based context fusion increases the memory footprint proportionally to conversation length. Does not address threshold calibration or cache management beyond basic LRU eviction. \\
\hline
6 & Liu et al., 2024 & Lost in the Middle & Provides a systematic analysis of how transformer-based language models attend to information at different positions within long input contexts. Demonstrates that models tend to underweight information placed in the middle of the context window. & The findings relate to how models process cached or retrieved context rather than how to decide when to serve cached responses. Not directly applicable to semantic cache threshold or admission decisions. \\
\hline
7 & Regmi \& Pun, 2024 & GPT Semantic Cache & Implements a Redis-backed semantic cache using HNSW approximate nearest-neighbor search for query matching. Demonstrates practical latency and cost reductions on real-world GPT API workloads with a fixed cosine similarity threshold of 0.75. & The fixed global threshold cannot adapt to the varying precision requirements of different cached entries. Uses basic LRU eviction that ignores entry regeneration cost, and admits all queries unconditionally. \\
\hline
8 & Jin et al., 2024 & RAGCache & Designs a caching layer specifically for retrieval-augmented generation pipelines, using cost-aware GDSF eviction that prioritizes retention of entries with high regeneration cost. Employs a conservative static threshold of 0.85 to maintain high precision for RAG outputs. & The static global threshold is overly conservative for many query types, reducing hit rate unnecessarily. No per-entry threshold adaptation or exploration mechanism to discover optimal threshold values. \\
\hline
9 & Schroeder et al., 2025 & vCache & Introduces per-entry conformal thresholds using online conformal prediction, providing formal guarantees on the false positive rate. Each cached entry maintains its own similarity boundary calibrated from feedback history. & Uses a relatively high default threshold of 0.80, which limits initial hit rates for new entries. Employs simple LRU eviction without cost awareness and lacks bandit-based exploration for learning optimal default thresholds. \\
\hline
10 & Zha et al., 2023 & Online Optimization for Cache & Applies Bayesian optimization to automatically tune the global similarity threshold for semantic cache systems. Demonstrates convergence to near-optimal threshold values on synthetic query distributions. & Optimizes a single global threshold rather than per-entry thresholds. Requires manual initialization and focuses on a single query type distribution, limiting applicability to heterogeneous workloads. \\
\hline
11 & Zhu et al., 2023 & Semantic Caching for LLM Cost & Proposes an embedding-based semantic cache lookup with a fixed similarity threshold of $\tau$=0.8 to reduce LLM API costs. Provides empirical cost savings analysis on OpenAI API workloads. & The fixed threshold does not adapt to varying query characteristics. No formal analysis of false positive rates, and the system lacks admission control or intelligent eviction beyond basic recency. \\
\hline
12 & Einziger et al., 2017 & TinyLFU & Proposes frequency-gated cache admission using a Count-Min Sketch for space-efficient frequency estimation. The admission gate rejects entries that lack sufficient access frequency, preventing cache pollution by one-hit wonders. & Designed for traditional web caching with exact key matching. Does not handle semantic similarity, embedding-based frequency estimation, or the unique challenges of LLM response caching. \\
\hline
13 & Bifet \& Gavalda, 2007 & ADWIN & Introduces adaptive windowing for detecting distribution changes in data streams using Hoeffding bounds. The window automatically shrinks when a statistically significant change is detected, discarding pre-change data. & A general-purpose drift detection algorithm that requires integration into a specific application context. Not directly designed for cache systems or threshold management, requiring adaptation for bandit posterior resetting. \\
\hline
14 & Vovk et al., 2005 & Conformal Prediction & Establishes the theoretical foundation for constructing prediction sets with guaranteed coverage under the exchangeability assumption. Provides nonparametric confidence regions that require no distributional assumptions beyond exchangeability. & The framework is theoretical and does not specify how to apply conformal prediction to per-entry cache threshold calibration. Requires adaptation to handle the streaming, per-entry nature of cache feedback signals. \\
\hline
15 & McMahan et al., 2017 & FedAvg & Proposes Federated Averaging for training deep neural networks across distributed clients by averaging locally trained model weights. Demonstrates communication efficiency by reducing the number of synchronization rounds needed for convergence. & Designed for gradient-based model training rather than bandit posterior synchronization. Application to cache threshold learning across distributed nodes requires adaptation of the averaging scheme to Beta distribution parameters. \\
\hline
...
\item \textbf{cache\_manager} -- Central orchestrator coordinating all cache operations, query pipeline, feedback processing, and statistics aggregation.
\item \textbf{lookup\_engine} -- Two-tier lookup with SHA-256 exact hash and FAISS IndexIDMap2(IndexHNSWFlat).
\item \textbf{embedding\_service} -- Sentence-transformers wrapper with LRU embedding cache.
\item \textbf{conformal\_thresholds} -- Per-entry conformal threshold store with online calibration.
\item \textbf{bandit} -- Thompson Sampling MAB with Beta posteriors and ADWIN integration.
\item \textbf{admission\_policy} -- Semantic W-TinyLFU with LSH-bucketed CMS frequency estimation.
\item \textbf{eviction} -- CostGDSF eviction with inflation factor tracking.
\item \textbf{drift\_detection} -- ADWIN drift detector using Hoeffding bounds.
\item \textbf{circuit\_breaker} -- Per-dependency circuit breaker (CLOSED/OPEN/HALF\_OPEN).
\item \textbf{session\_context} -- Session context accumulator with EMA fusion.
\item \textbf{llm\_provider} -- Unified LLM abstraction over Ollama, Groq, and OpenAI.
\item \textbf{redis\_store} -- Optional Redis-backed exact-match store with circuit breaker.
\item \textbf{persistence} -- SQLite persistence for cache entries, scores, bandit state, and chat history.
\item \textbf{metrics} -- Metrics collector powering the dashboard.
\item \textbf{data\_structures} -- CMS, LSH, LRU, and SLRU implementations.
\item \textbf{config} -- Pydantic BaseSettings with CC\_ environment variable overrides.
\end{itemize}
...
\begin{enumerate}[label=(\roman*),leftmargin=2cm]
\item \textbf{Exact-Match LRU} -- Uses SHA-256 hashing of normalized query text and matches only identical queries. Eviction follows a standard LRU policy with the same capacity as all other strategies. This baseline serves as a lower bound, quantifying the cache performance achievable without any semantic matching capability. Any improvement over this baseline is directly attributable to the semantic similarity mechanism.

\item \textbf{GPTCache~[3]} -- Implements a fixed cosine similarity threshold of 0.75 with HNSW-based approximate nearest-neighbor search for query matching. Cache management uses LRU eviction with no admission control, meaning every cache miss results in unconditional entry insertion. This represents the most widely adopted open-source semantic cache design and serves as the primary point of comparison for our system.

\item \textbf{MeanCache~[2]} -- Computes an adaptive global threshold as the mean minus one standard deviation of observed similarity scores, which adjusts to the overall distribution of query similarities. Embeddings are compressed using PCA before storage, and eviction follows LRU ordering. The federated learning component for similarity model training is not used in this single-node benchmark, isolating the caching policy for fair comparison.

\item \textbf{vCache~[4]} -- Maintains per-entry conformal thresholds using online conformal prediction with a target error rate matching our system. The default threshold for new entries is set higher at 0.80, and eviction uses simple LRU without cost awareness. This is the closest related work to our system and isolates the contribution of Thompson Sampling exploration, CostGDSF eviction, and W-TinyLFU admission.

\item \textbf{RAGCache~[8]} -- Uses a conservative static similarity threshold of 0.85 designed for retrieval-augmented generation workloads where serving an incorrect cached document can cascade into factually wrong answers. Eviction follows CostGDSF, similar to our approach, prioritizing retention of entries with high regeneration cost. This baseline tests whether per-entry threshold adaptation provides value beyond cost-aware eviction alone.

\item \textbf{No-Admission} -- An ablation of the full ContextualCache system with the W-TinyLFU admission gate removed. It retains per-entry conformal thresholds and CostGDSF eviction but admits every cache miss entry unconditionally. Comparing this against the full system isolates the contribution of frequency-gated admission to overall hit rate and precision.
\end{enumerate}
...
Every tunable parameter in the system is exposed through a Pydantic BaseSettings configuration class that reads values from environment variables prefixed with \texttt{CC\_} or from a \texttt{.env} file at startup. This design makes it straightforward to adjust the cache behavior without touching any source code, which is particularly useful when switching between development, benchmarking, and production deployments. For instance, the benchmark runs use a cache capacity of 300 entries to stress the eviction and admission logic under tight constraints, while the default production setting of 50,000 entries provides ample headroom for real workloads. The conformal error rate, embedding model, HNSW index parameters, and bandit arm count can all be changed independently, allowing operators to trade off between precision, recall, latency, and memory usage depending on what matters most for their particular use case. Table~\ref{tab:config} lists the key parameters along with their default values.
...
\begin{enumerate}[label=(\roman*),leftmargin=2cm]
\item \textbf{Natural Questions (NQ)~[17]} -- Google's Natural Questions dataset containing web search queries with short answers. 500 unique questions were sampled with 150 paraphrase variants (30\% ratio) for a total of 650 queries per run.
\item \textbf{SQuAD~[18]} -- Stanford Question Answering Dataset (validation split) containing reading comprehension questions. 500 unique questions with 150 paraphrases, totaling 650 queries.
\end{enumerate}
...
\midrule
\textbf{ContextualCache} & \textbf{20.77\%} & \textbf{62.22\%} & \textbf{0.214} & \textbf{84} & \textbf{51} & \textbf{515} \\
...
\bottomrule
\end{tabular}
\caption[Benchmark Results on Natural Questions Dataset]{Benchmark Results on Natural Questions Dataset \\ (650 queries, 300 capacity)}
\label{tab:nq}
\end{table}
...
Figure~\ref{fig:conformal} illustrates how individual conformal thresholds evolve over the course of the benchmark as feedback signals accumulate. All five sample entries begin at the default threshold of 0.70, indicated by the gray dashed line. After receiving the minimum three feedback events required for calibration, each entry's threshold begins to diverge based on the nature of the queries it serves. Entry A, which handles precise factual queries where incorrect cached responses are easily distinguishable, sees its threshold rise to approximately 0.85 as the conformal quantile tightens to reduce false positive risk. Entry C, which corresponds to a broad topical query where most semantically similar queries are indeed valid matches, drops its threshold toward the lower clamp bound of 0.65, allowing the system to serve cached responses more aggressively for that particular entry.

This per-entry adaptation is what distinguishes the proposed system from global-threshold approaches. Rather than applying a single compromise threshold across all entries, each cached response learns its own similarity boundary from the feedback it receives. The result is a heterogeneous threshold landscape where precise entries are strict and broad entries are permissive, both operating within the formal conformal guarantee that the per-entry false positive rate remains below the target error rate of five percent.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig-conformal-evolution.png}
\caption{Per-Entry Conformal Threshold Evolution Over Feedback Events}
\label{fig:conformal}
\end{figure}
...
\textbf{Strategy} & \textbf{Hit Rate} & \textbf{Precision} & \textbf{F1} & \textbf{Correct} & \textbf{Incorrect} & \textbf{LLM Calls} \\
\midrule
\textbf{ContextualCache} & \textbf{23.69\%} & 35.71\% & \textbf{0.137} & \textbf{55} & 99 & \textbf{496} \\
...
\bottomrule
\end{tabular}
\caption[Benchmark Results on SQuAD Dataset]{Benchmark Results on SQuAD Dataset \\ (650 queries, 300 capacity)}
\label{tab:squad}
\end{table}
...
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \textbf{GPTCache} & \textbf{vCache} & \textbf{ContextualCache} \\
\midrule
...
Figure~\ref{fig:simdist} shows the distribution of cosine similarity scores for correct and incorrect cache hits on the NQ dataset. Correct hits cluster at higher similarity values with a mean around 0.82, indicating that the embedding model assigns noticeably higher similarity to queries that are genuine semantic matches. Incorrect hits are spread more broadly with a mean around 0.74, overlapping with the lower end of the correct distribution. The vertical dashed line at 0.70 marks the default threshold. The overlap region between the two distributions is where the per-entry conformal thresholds operate most critically, learning for each individual entry whether a given similarity score is likely to produce a correct or incorrect response.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig-similarity-distribution.png}
\caption{Distribution of Cosine Similarity Scores for Correct and Incorrect Cache Hits}
\label{fig:simdist}
\end{figure}
...
\midrule
ContextualCache & 0.00 & 1610.17 & 135 \\
...
The Thompson Sampling bandit provides insights into the optimal threshold region. After processing the NQ benchmark queries, the arm distribution reveals the learned expected rewards for each of the ten threshold values, as shown in Figure~\ref{fig:thompson}. Arms corresponding to lower thresholds in the 0.65 to 0.75 range accumulate higher expected rewards because these thresholds allow the cache to serve responses for a broader set of semantically similar queries, many of which turn out to be correct. The best arm, highlighted in orange, corresponds to a threshold near 0.65 with an expected reward of approximately 0.89, confirming that the bandit learns to favor more permissive defaults for this particular workload.

As the threshold increases toward the conservative end of the spectrum, the expected reward drops sharply. Arms at 0.85 and above rarely match any queries at all, so they accumulate far fewer success signals and their Beta posteriors remain pessimistic. This monotonically decreasing reward pattern is consistent with the observation that the NQ dataset contains genuine paraphrase variants that cluster at moderate similarity levels between 0.70 and 0.85, well below the thresholds used by conservative systems like RAGCache.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig-thompson-convergence.png}
\caption{Thompson Sampling Arm Expected Rewards after NQ Benchmark}
\label{fig:thompson}
\end{figure}
...
\textbf{Solution:} A periodic halving mechanism keeps the Count-Min Sketch focused on recent activity by reducing all counters after a fixed number of insertions (typically one cache cycle). This lets newer queries matter more while older ones naturally fade, maintaining adaptability without making frequency estimates unstable.
...
% ════════════════════════════════════════════════════════════
% Appendix B: Code Snippets and Tables
% ════════════════════════════════════════════════════════════
\newpage
\chapter*{Appendix B: Code Snippets}
\addcontentsline{toc}{section}{Appendix B: Code Snippets}

\noindent\textbf{Key Implementation Code Snippets}

\vspace{0.5cm}

\noindent The following code excerpts are taken from the core modules of the ContextualCache backend. They illustrate the four principal algorithmic strategies that distinguish the system from existing approaches.

\vspace{0.5cm}
\noindent\textbf{1. Conformal Threshold Retrieval} (\texttt{conformal\_thresholds} module)

\noindent This method computes the per-entry conformal threshold as the $(1 - \varepsilon)$ quantile of historical nonconformity scores. Entries with fewer than the minimum calibration points fall back to the default threshold.
...
async def get_threshold(self, entry_id: str) -> float:
    async with self._lock:
        ...
        return float(np.clip(threshold, self.min_threshold,
                             self.max_threshold))
\end{lstlisting}

\vspace{0.5cm}
\noindent\textbf{2. Thompson Sampling Update with ADWIN Drift Detection} (\texttt{bandit} module)

\noindent The bandit posterior is updated after each cache hit. The reward signal is fed to the ADWIN drift detector, and a detected distribution shift triggers a full posterior reset to weakly informative priors.

\begin{lstlisting}[language=Python]
def update(self, arm: int, reward: float) -> None:
    if reward == 0.5:
        return  # uncertain feedback -- skip
    ...
    if self.drift_detector.detected_change():
        logger.warning(
            "Distribution drift detected on shard %s "
            "-- resetting posteriors.", self.shard_id)
...
\vspace{0.5cm}
\noindent\textbf{3. Semantic W-TinyLFU Admission Decision} (\texttt{admission\_policy} module)

\noindent The admission gate records the access frequency of a new entry and compares it against the window cache LRU victim. Entries with higher frequency displace the victim and are admitted.
...
def should_admit(self, entry: CacheEntry) -> bool:
    ...
    if isinstance(victim_entry, CacheEntry):
        victim_freq = self._estimate_frequency(
            victim_entry.embedding)
    else:
        victim_freq = 0
    ...
    if new_freq >= victim_freq:
        ...
        if isinstance(victim_entry, CacheEntry):
            self.main_cache.put(
                victim_entry.entry_id, victim_entry)
        self.window_cache.put(entry.entry_id, entry)
        self.total_admitted += 1
        return True
...
\vspace{0.5cm}
\noindent\textbf{4. CostGDSF Eviction} (\texttt{eviction} module)

\noindent The eviction policy selects the entry with the lowest priority score. The inflation factor $L$ is updated to the evicted entry's priority, preventing newly inserted entries from being immediately evicted.
...
def evict_one(self) -> Optional[str]:
    ...
    victim_id = min(self._entries,
                    key=lambda eid: self._entries[eid].H)
